    ...
    <section className="relative flex flex-col justify-center py-12 px-4 md:px-8">
      ...
      >
        ...
        >
          ...
          que geram impacto real. Estou sempre buscando novos desafios e aprendizados para crescer
          ...
          {skills.map((skill) => {
            ...
            return (
              <motion.div
                key={skill.name}
                variants={item}
                className="p-6 rounded-lg bg-background/20 backdrop-blur-[1px] border border-muted/5 hover:border-primary/20 transition-colors group"
              >
                <Icon className="w-8 h-8 text-primary mb-4 group-hover:text-accent transition-colors" />
                <h3 className="text-xl font-semibold mb-2 text-foreground/90">{skill.name}</h3>
